...
export async function buscarLivroPorTitulo (titulo: string): Promise<Livro> {
    ...
    if (!dados) {
        throw new Error (`Livro com o titulo ${titulo} não encontrado.`);
    }
...
export async function atualizarDadosLivro (id: number, titulo: string, fkAutor: number, quantidadeTotal: number, quantidadeDisponivel: number): Promise<Livro> {
    ...
    if (quantidadeDisponivel > quantidadeTotal) {
        throw new Error (`Quantidade disponível não pode ser maior que a total de livros: ${quantidadeTotal}.`);
    }
...
function validarTitulo (titulo: string): void {
    ...
    if (!titulo || titulo === '') {
        throw new Error ('O nome do titulo do livro é obrigatório.');
    }

    if (titulo.trim().length > 200) {
        throw new Error ('O nome do livro não pode ultrapassar 200 caracteres.');
    }
}
...
async function validarAutorExistente (fkAutor: number): Promise<void> {
    ...
    if (!autor) {
        throw new Error (`Não é possível cadastrar o livro: o autor com id ${fkAutor} não existe.`);
    }
...
async function validarLivroNaoDuplicado(titulo: string): Promise<void> {
  ...
  if (existente) {
    throw new Error(`Esse autor já tem um livro cadastrado com o título "${titulo}".`);
  }
}
...
function validarQuantidade (quantidade: number): void {
    ...
    if (!Number.isInteger(quantidade) || quantidade < 0) {
        throw new Error ('A quantidade total deve ser um número inteiro maior ou igual a zero.');
    }
}
